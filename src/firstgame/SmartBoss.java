...
public class SmartBoss extends GameObject{
    ...
    public void tick(){
        ...
        if(timer2 <= 0) {
            ...
            if(spawn == 0) handler.addObject(new SmartBossBullet((int)x + 48, (int)y + 48, ID.SmartEnemy, handler));
        }
        
        //if(y <= 0 || y >= FirstGame.HEIGHT - 48) velY *= -1;
        if(x <= 0 || x >= FirstGame.WIDTH - 108) velX *= -1;
        
       handler.addObject(new Trail(x, y, ID.Trail, Color.cyan, 96, 96, 0.09f, handler));
    }
